...
export async function getVoiceAssignments(scriptId, userId) {
  try {
    ...
    console.log('🔍 Get voice assignments raw response:', responseText);
    ...
    try {
      ...
      console.log('🔍 Parsed get voice assignments result:', result);
    } catch (parseError) {
    ...
    if (result && result.data && Array.isArray(result.data.assignments)) {
      console.log('🔍 Using data.assignments format for get response');
      ...
      console.log('🔍 Using assignments array format for get response');
      result.assignments.forEach(assignment => {
        assignments[assignment.character_name] = assignment.cartesia_voice_id;
      });
    } else if (result && Array.isArray(result)) {
      console.log('🔍 Using direct array format for get response');
      result.forEach(assignment => {
        assignments[assignment.character_name] = assignment.cartesia_voice_id;
      });
    } else {
      console.warn('🔍 Unknown get voice assignments response format:', result ? Object.keys(result) : result);
    }
    
    console.log('🔍 Final converted assignments:', assignments);
...
export async function saveVoiceAssignments(scriptId, assignments, userId, productionId = null) {
  try {
    ...
    console.log('💾 Sending save request:', JSON.stringify(payload, null, 2));
    console.log('📡 Webhook URL:', import.meta.env.VITE_N8N_SAVE_VOICE_ASSIGNMENTS_WEBHOOK);
...
export async function autoAssignVoices(scriptId, userId, characters = []) {
  try {
    console.log('🎭 Auto-assign called with characters:', characters);
    ...
    console.log('🔍 Auto-assign payload being sent:', JSON.stringify(payload, null, 2));
    console.log('🔍 Auto-assign webhook URL:', import.meta.env.VITE_N8N_AUTO_ASSIGN_VOICES_WEBHOOK);
    ...
    const responseText = await response.text();
    console.log('🔍 Auto-assign raw response:', responseText);
    ...
    try {
      result = JSON.parse(responseText);
      console.log('🔍 Auto-assign parsed result:', result);
    } catch (parseError) {
